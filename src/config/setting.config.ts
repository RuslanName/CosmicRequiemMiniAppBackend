import { DEFAULT_SETTINGS, TIME_UNIT_MULTIPLIERS } from '../modules/setting/setting-constants';
...
export const reloadSettings = async () => {
    ...
    const dbMap = Object.fromEntries(dbSettings.data.map((s) => [s.key, s.value]));
    ...
    for (const key of Object.values(SettingKey)) {
        if (dbMap[key] !== undefined) {
            ...
            if (typeof def === 'number') {
                ...
                (Settings as any)[key] = ['true', '1', 'yes', 'on'].includes(raw.toLowerCase().trim());
