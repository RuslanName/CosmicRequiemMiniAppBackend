...
export class AdminService {
    ...
    async findAll(paginationDto: PaginationDto): Promise<{ data: Admin[]; total: number; page: number; limit: number }> {
    ...
    async create(createAdminDto: CreateAdminDto): Promise<Admin> {
        ...
        if (existingAdmin) {
            throw new BadRequestException('Admin with this username or user_id already exists');
        }
    ...
    async createSystemAdmin(vkId: number, username: string, password: string, userId: number): Promise<Admin> {
        const existingAdmin = await this.adminRepository.findOne({
            where: { username },
        });
