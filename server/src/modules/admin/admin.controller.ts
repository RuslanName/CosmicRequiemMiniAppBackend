...
export class AdminController {
    ...
    @ApiResponse({ status: 200, description: 'Возвращает список администраторов с пагинацией' })
    async findAll(@Query() paginationDto: PaginationDto): Promise<{ data: Admin[]; total: number; page: number; limit: number }> {
        return this.adminService.findAll(paginationDto);
    }

    @Get('me')
    @ApiOperation({ summary: 'Получить текущего аутентифицированного администратора' })
    @ApiResponse({ status: 200, description: 'Возвращает текущего администратора' })
